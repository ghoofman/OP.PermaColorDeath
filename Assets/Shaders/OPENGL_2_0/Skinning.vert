...
void main() {
	...
	float dist = distance(newPosition.xyz, vec3(0,0,0));
	float diff = abs(dist - (uRipple * speed));
	float mod = clamp(1.0 - (diff * diff)/(width * width), 0.0, 1.0);

	vColor = uColorTo;
	if(dist - (uRipple * speed) < 0.0) {
		vColor = uColor;
	}

	newNormal = normalize(newNormal);
	newPosition += (newNormal * mod * amp);
	...
	//norm.a = 1.0;
	vNormal = ( uView * uWorld * vec4(newNormal.xyz,0)).xyz; // Onl
	//vNormal = newNormal.xyz; // Only correct if ModelMatrix does not scale the model ! Use its inverse transpose if not.
